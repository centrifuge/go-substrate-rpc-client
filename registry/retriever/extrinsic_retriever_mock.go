...
func (_m *ExtrinsicRetrieverMock) GetExtrinsics(blockHash types.Hash) ([]*parser.Extrinsic, error) {
	ret := _m.Called(blockHash)

	var r0 []*parser.Extrinsic
	if rf, ok := ret.Get(0).(func(types.Hash) []*parser.Extrinsic); ok {
		r0 = rf(blockHash)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]*parser.Extrinsic)
		}
	}
