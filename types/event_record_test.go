...
func TestEventRecordsRaw_Decode(t *testing.T) {
	...
	events := EventRecords{}
	err := e.DecodeEventRecords(ExamplaryMetadata11Substrate, &events)
	if err != nil {
		panic(err)
	}

	exp := EventRecords{
		Balances_Endowed:                   []EventBalancesEndowed(nil),
		Balances_DustLost:                  []EventBalancesDustLost(nil),
		...
		Indices_IndexAssigned:              []EventIndicesIndexAssigned(nil),
		Indices_IndexFreed:                 []EventIndicesIndexFreed(nil),
		...
		System_CodeUpdates:                 []EventSystemCodeUpdated(nil),
		System_NewAccount:                  []EventSystemNewAccount(nil),
		System_KilledAccount:               []EventSystemKilledAccount(nil),
	} //nolint:lll
