...
func TestExtrinsicSignatureV3_EncodeDecode(t *testing.T) {
	...
	sigEnc, err := EncodeToHexString(sig, scale.EncoderOptions{})
...
func TestExtrinsicSignatureV4_EncodeDecodeWithOpts(t *testing.T) {
	...
	sigEnc, err := EncodeToHexString(sig, scale.EncoderOptions{NoPalletIndices: true})
	assert.NoError(t, err)

	var sigDec ExtrinsicSignatureV4
	err = DecodeFromHexString(sigEnc, &sigDec, scale.EncoderOptions{NoPalletIndices: true})
	assert.NoError(t, err)
